...
BEGIN
   ...
   v_tolerancia_cobro = pxp.f_get_variable_global('tolerancia_cobro_amadeus');
    ...
    if ((v_boleto.total-v_boleto.comision) > round(v_acumulado_moneda_boleto,2)) then
    	if (((v_boleto.total-v_boleto.comision) - round(v_acumulado_moneda_boleto,2)) > v_tolerancia_cobro) then
        	raise exception 'El total a pagar definido en las formas de pago:% ,no iguala al total del boleto : %',v_acumulado_moneda_boleto,(v_boleto.total-v_boleto.comision);
		end if;
    elsif ((round(v_acumulado_moneda_boleto,2) - (v_boleto.total-v_boleto.comision)) > 1) then

    	raise exception ' El total a pagar definido en las formas de pago:% , es mayor al total del boleto: %',v_acumulado_moneda_boleto,(v_boleto.total-v_boleto.comision);
    end if;
