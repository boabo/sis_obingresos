  ...
  BEGIN
    ...
    ***********************************/
      ...
      begin
        ...
        /*Aumantando Condicion para que el sistema recupere informacion de la nueva tabla obingresos.tboleto_stage y obingrseos.tboleto_forma_pago_stage*/


        if (v_parametros.fecha_ini::date >= '01/03/2021') then

        v_consulta:='(select to_char(b.fecha_emision,''DD/MM/YYYY'')::varchar,sum(bfp.importe),''ingresos''::varchar as tipo
                      from obingresos.tboleto_stage  b
                      inner join obingresos.tboleto_forma_pago_stage bfp on b.id_boleto =bfp.id_boleto
        ...
        v_consulta:='
                    with
                        anulados as (select max(b.id_boleto),b.localizador,b.fecha_emision,
                                    ...
                                    inner join obingresos.tboleto_stage b on b.nro_boleto = vwm.nro_boleto and b.localizador is not null and trim(both '' '' from b.localizador) != ''''
                        ...
                        reemision as (select max(bree.id_boleto),bree.localizador,bree.fecha_emision,(''PNR : '' || coalesce(banu.localizador,''"no identificado"'') || coalesce(''('' || banu.fecha_emision || '')'',''()'') ||'' anulado y  reemitido con PNR ''||bree.localizador)::varchar as observaciones
                                    from obingresos.tventa_web_modificaciones vwm
                                    inner join obingresos.tboleto_stage bree on bree.nro_boleto = vwm.nro_boleto_reemision and bree.estado_reg = ''activo''
                                    left join obingresos.tboleto_stage banu on banu.nro_boleto = vwm.nro_boleto and banu.localizador is not null and
...
ALTER FUNCTION obingresos.ft_detalle_boletos_web_sel (p_administrador integer, p_id_usuario integer, p_tabla varchar, p_transaccion varchar)
  OWNER TO postgres;
